    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modal-materia-prima-title">Nueva Materia Prima</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="modal-materia-prima-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8 ">
                        <input type="text" class="form-control form-control-sm" placeholder="Ingrese Nombre"
                            v-model="materiaPrima.descripcion"/>
                        <small class="text-danger font-weight-bold" v-for="error in errores.descripcion">@{{ error }}</small>
                    </div>
                </div>
                <div class="form-group text-center">
                    <button type="submit" class="btn btn-success btn-guardar-materia" :value='materiaPrima.estadoCrud'
                            name="btn-guardar-materia" @click.prevent="guardar">
                            <i :class="(materiaPrima.estadoCrud == 'nuevo') ? 'fa fa-save' : 'fa fa-refresh'"></i>  
                        @{{ ( materiaPrima.estadoCrud == 'nuevo' ? 'Guardar' : 'Actualizar')}}
                    </button> 
                    &nbsp;
